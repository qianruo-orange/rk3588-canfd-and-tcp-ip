                                   ...
                                   uint64_t bmdt90, uint32_t dur90)
{
    ...
    /* FullBox 首字节是 version，低 3 字节才是 flags：必须写 0x01000000。
       写成 1 会被解析为 version=0/flags=1，即声明 32 位时间却带了 8 字节负载，
       解析器只取 64 位值的高 4 字节——恒为 0，于是每个片段都自称起始于 0，
       样本全部叠在时间轴原点（ffmpeg 只解出 1 帧，浏览器缓冲不推进） */
    h264s_bb_u32(&b, 0x01000000u);           /* version 1 → 64 位（32 位 90000 刻度 ~13.3h 回绕） */
