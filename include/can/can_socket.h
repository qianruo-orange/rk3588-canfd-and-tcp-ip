...
/* ---- CAN 收发队列 ----
 * RX：can_task 从 socket 读到帧先入 rxq，随后在本轮内排空并回调 on_can_rx（无独立消费线程）。
 * TX：业务线程压入待发送帧（can_queue_t 接口），can_task 弹出并写 socket；用 tx_efd 唤醒 epoll。
 * 所有接口共用一个发送队列、一个接收队列；帧内 ifname 字段标识来源/目标接口。 */
...
typedef struct can_ctx {
    ...
    can_queue_t      txq;       /* 所有接口共用的发送队列 */
    can_queue_t      rxq;       /* 所有接口共用的接收队列 */
 ...
 * 帧内 ifname 为目标接口名。返回 0 表示已入队（稍后发送），>0 表示立即发送字节数，-1 失败。 */
